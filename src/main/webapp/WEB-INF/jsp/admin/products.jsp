...
<div class="card p-3 mb-3">
    <form method="get" action="${pageContext.request.contextPath}/admin/products" class="row g-2 align-items-end">
        <div class="col-md-5">
            <label class="form-label mb-1">Поиск по ID или названию</label>
        ...
        <div class="col-md-2"><a class="btn btn-outline-secondary w-100" href="${pageContext.request.contextPath}/admin/products">Сброс</a></div>
    </form>
</div>
    ...
    <table class="table table-sm table-hover align-middle">
        ...
        <c:forEach items="${products}" var="product">
            <tr>
                ...
                <td>${categoryНазваниеs[product.categoryId]} <small class="text-muted">(ID: ${product.categoryId})</small></td>
                <td class="text-truncate" style="max-width:180px;">${product.imageUrl}</td>
                <td>
                    <button class="btn btn-sm btn-outline-primary" type="button" data-bs-toggle="collapse" data-bs-target="#edit-product-${product.id}" aria-expanded="false" aria-controls="edit-product-${product.id}">Редактировать</button>
                </td>
                <td>
