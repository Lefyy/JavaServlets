...
<div class="card p-3 mb-3">
    <form method="get" action="${pageContext.request.contextPath}/admin/order-statuses" class="row g-2 align-items-end">
        ...
        <div class="col-md-2"><a class="btn btn-outline-secondary w-100" href="${pageContext.request.contextPath}/admin/order-statuses">Сброс</a></div>
    </form>
</div>
...
<table class="table table-sm table-hover">
    ...
    <c:forEach items="${statuses}" var="status">
        <tr>
            <td>${status.id}</td>
            <td>${status.name}</td>
            <td><button class="btn btn-sm btn-outline-primary" type="button" data-bs-toggle="collapse" data-bs-target="#edit-status-${status.id}" aria-expanded="false" aria-controls="edit-status-${status.id}">Редактировать</button></td>
            <td>
