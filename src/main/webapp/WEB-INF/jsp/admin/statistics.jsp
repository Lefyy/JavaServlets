...
<form method="get" action="${pageContext.request.contextPath}/admin/statistics" class="card shadow-sm mb-4">
    <div class="card-body">
        <div class="row g-3 align-items-end">
            <div class="col-md-3"><label class="form-label" for="from">От</label><input type="date" class="form-control" id="from" name="from" value="${fromDate}"></div>
            <div class="col-md-3"><label class="form-label" for="to">До</label><input type="date" class="form-control" id="to" name="to" value="${toDate}"></div>
            <div class="col-md-3"><label class="form-label" for="period">Быстрый период</label><select class="form-select" id="period" name="period"><option value="">Выберите период</option><option value="day">За день</option><option value="week">За неделю</option><option value="month">За месяц</option><option value="year">За год</option></select></div>
            <div class="col-md-3 d-flex gap-2"><button type="submit" class="btn btn-primary">Применить</button><a href="${pageContext.request.contextPath}/admin/statistics" class="btn btn-outline-secondary">Сбросить</a></div>
        </div>
    </div>
</form>
<div class="row g-3">
